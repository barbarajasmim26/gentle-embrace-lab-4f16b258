...
import { Send, MessageCircle, ExternalLink, CheckCircle2, Pencil, RotateCcw, Save, X, Info } from "lucide-react";
...
export default function WhatsAppPage() {
  ...
  const handleSendIndividual = () => {
    ...
    if (!t?.phone) {
      toast.error("Selecione um inquilino com telefone.");
      return;
    }
  ...
  const previewMessage = mode === "individual" && tenant ? getMessageForTenant(singleTenant) : "";

  const startEditing = (key: TemplateKey) => {
    setEditingKey(key);
  ...
  const saveEditing = () => {
    if (editingKey) {
      updateTemplate(editingKey, editDraft);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-2">
        ...
        <Button variant={mode === "mass" ? "default" : "outline"} onClick={() => setMode("mass")}>
          Em Massa
        </Button>
      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">
                {mode === "individual" ? "Selecionar Inquilino" : "Selecionar Inquilinos"}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
                    ...
                    {tenants?.map((t) => (
                      <label key={t.id} className="flex items-center gap-2 text-sm">
                        <Checkbox
                          ...
                          disabled={!t.phone}
                        />
                        <span className={!t.phone ? "text-muted-foreground" : ""}>
              ...
              <div>
                ...
                <Select value={template} onValueChange={(v) => setTemplate(v as TemplateKey | "custom")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(Object.entries(TEMPLATE_LABELS) as [TemplateKey, string][]).map(([k, v]) => (
                      <SelectItem key={k} value={k}>
                        <span className="flex items-center gap-2">
                          {v}
                          {isModified(k) && <Badge variant="secondary" className="text-[10px] px-1 py-0">editado</Badge>}
                        </span>
                      </SelectItem>
                    ))}
              ...
              {mode === "individual" && (
                <Button className="w-full" onClick={handleSendIndividual}>
            ...
            <Card>
                      ...
                      <TooltipContent side="left" className="max-w-xs">
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs">
                          {TEMPLATE_VARIABLES.map((v) => (
                            <span key={v.tag}>
                              <code className="text-primary">{v.tag}</code> {v.desc}
                            </span>
                          ))}
                        </div>
                      </TooltipContent>
              ...
              <CardContent className="space-y-3">
                {editingKey === template ? (
                  ...
                  <>
                    <div className="bg-muted rounded-lg p-3 text-sm whitespace-pre-wrap font-mono min-h-[100px]">
                      {templates[template]}
                    </div>
                    <div className="flex gap-2">
                      ...
                      {isModified(template) && (
                        <Button size="sm" variant="ghost" onClick={() => handleResetTemplate(template)}>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between gap-2 p-3 rounded-lg border ${
                          isSent ? "bg-green-500/10 border-green-500/30" : "bg-muted/50"
                        }`}
                      >
